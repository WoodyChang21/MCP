...
async def get_enhanced_prompt(user_input: str) -> str:
    ...
    try:
        ...
        - ALWAYS embed Tako visualizations using iframe format: <iframe src="EMBED_URL" width="100%" height="400"></iframe>
        - When you get visualization results from Tako tools, extract the embed URL and display it as an iframe
        - Do NOT just show the URL as text - always embed it for interactive viewing
        - If multiple visualizations are available, embed each one separately
        - Use the embed URL (not the image URL) for interactive charts
        - Example format: <iframe src="https://trytako.com/embed/abc123" width="100%" height="400"></iframe>
        ...
        - Prioritize interactive visualizations over static images
        """
